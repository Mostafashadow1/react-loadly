...
export function AutoSkeletonRoadmapSection() {
  const stages = ["Experimental", "Compiler Research", "Prototype", "Public Beta", "Stable"];
  const goals = [
    "Better accuracy",
  ...
  return (
    <section id="roadmap" className="relative border-t border-border/20 bg-background py-24">
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_0.8fr] lg:items-start">
          <div>
            ...
            <p className="mt-4 text-muted-foreground">
              ...
              Instead of relying on runtime component introspection, the new architecture will
              leverage compile-time analysis to generate more accurate skeleton structures.
            </p>
            <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2">
              {goals.map((goal) => (
                <div key={goal} className="rounded-lg border border-zinc-800 bg-zinc-950/40 px-4 py-3 text-sm text-zinc-300">
                  {goal}
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-col items-center gap-3">
              {stages.map((stage, index) => (
                <div key={stage} className="flex w-full flex-col items-center gap-3">
                  ...
                  {index < stages.length - 1 && <ArrowDown className="size-4 text-zinc-600" />}
                </div>
              ))}
            </div>
          </div>
