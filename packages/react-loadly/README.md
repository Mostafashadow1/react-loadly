...
🏠 **Home Page**: [https://mostafashadow1.github.io/react-loadly/](https://mostafashadow1.github.io/react-loadly/)
...
React Loadly is no longer positioned as an automatic skeleton generator. It is a **Reliable React Loading Experience Toolkit** built around:
...
- Production-ready loading patterns
...
These patterns are predictable, easy to design-review, compatible with complex React trees, and safer for production UI than runtime component introspection.
...
## AutoSkeletonLoader Experimental

`AutoSkeletonLoader` is currently **experimental**.

The current implementation remains available for experimentation and backward compatibility. It uses best-effort structural inference by combining JSX traversal, safe presentational component inference, `className` heuristics, and smart fallback presets to generate realistic loading states.
...
In this example, we use a presentational card and let `AutoSkeletonLoader` infer a matching skeleton with zero extra skeleton markup.

```tsx
import { AutoSkeletonLoader } from "react-loadly";
import "react-loadly/styles.css";

// A presentational component with memo and sub-components
...
### AutoSkeleton Examples

```tsx
<AutoSkeletonLoader component={<ProfileCard />} />
<AutoSkeletonLoader component={<ProductCard />} />
<AutoSkeletonLoader component={<DashboardCard />} />
```
...
## New Production Loaders

### GradientRingLoader

A premium gradient ring spinner for polished product surfaces and dashboards.
...
Accessibility: renders `role="status"`, `aria-live="polite"`, and a screen-reader label.

### SignalLoader

Animated signal bars for network, streaming, and realtime data loading states.
...
Orbiting particles for polished loading states in dashboards, media apps, and realtime interfaces.
...
Audio-style animated bars for uploads, streams, media, and activity states.
...
A subtle flipping card animation for card-heavy interfaces and content loading.
...
- **AutoSkeletonLoader Experimental**: Best-effort runtime skeleton generation for experimentation and backward compatibility.

---

## AutoSkeleton Compiler (In Progress)

We are actively building a next-generation compiler-assisted AutoSkeleton engine.

Instead of relying on runtime component introspection, the new architecture will leverage compile-time analysis to generate more accurate skeleton structures.
...
The showcase app is the source of truth for examples.
...
6. **CSS Containment & Performance** - Added `contain: layout style paint;` to skeleton container styles to reduce browser reflows during shimmer animations.
7. **Rules of Hooks Compliance** - Refactored all 30 loader components to ensure hooks are called unconditionally before early returns.
...
```jsx
// Import hooks
import { useSkeletonState } from "react-loadly/hooks";

// Import skeleton components
import { AutoSkeletonLoader } from "react-loadly/skeleton";

// Import loaders
import { SpinLoader } from "react-loadly/loaders";
```
...
### Global Configuration: `LoadlyProvider`

The `LoadlyProvider` allows you to customize defaults globally, manage theme modes (`"light"` | `"dark"`), and handle automatic CSS injection:

```tsx
import { LoadlyProvider } from "react-loadly";
import { SpinLoader } from "react-loadly/loaders";

function App() {
  return (
    <LoadlyProvider
      theme="dark" // Sets dark mode defaults automatically
      color="#8b5cf6" // Overrides the default loader color globally
      injectStyles={true} // Automatically injects base CSS into the document head
    >
      <SpinLoader />
    </LoadlyProvider>
  );
...
| Prop | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `theme` | `"light" \| "dark"` | `"light"` | Configures automatic color adjustments for theme modes. |
| `color` | `string` | `"#6366f1"` | Global base loader color. |
| `secondaryColor` | `string` | `"#8b5cf6"` | Global secondary loader color. |
| `speed` | `number` | `1` | Global animation speed multiplier. |
| `injectStyles` | `boolean` | `true` | When true, automatically injects base CSS into the document head at runtime. |
| `loaderCenter` | `boolean` | `false` | Centers all loaders horizontally and vertically when fullscreen is enabled. |
...
| Prop | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `stagger` | `number` | `0.1` | Delay in seconds between the start of each skeleton child's shimmer. |
| `shimmerSync` | `boolean` | `true` | Coordinates animation timeline phases across all children. |

---

### `useSkeletonState` Hook

`useSkeletonState` is a high-level orchestrator hook designed to manage loading, skeleton props, and error/retry states with a `minDisplayTime` stabilizer to prevent jarring UI flashes.
...
function UserProfile({ userId }) {
  const { loading, error, showSkeleton, retry, data } = useSkeletonState({
    fetchFn: () => fetchUser(userId),
    minDisplayTime: 400, // Keeps the skeleton visible for at least 400ms to stabilize layout
  });

  if (error) {
    return <button onClick={retry}>Retry loading profile</button>;
  }

  return (
    <AutoSkeletonLoader
      loading={showSkeleton}
      component={<ProfileCard user={data} />}
    />
  );
}
```

#### `useSkeletonState` Options

| Option | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `fetchFn` | `() => Promise<T>` | **Required** | The async function to fetch data. |
| `minDisplayTime` | `number` | `300` | Minimum duration (in milliseconds) the skeleton will remain visible to prevent flashing. |
| `onSuccess` | `(data: T) => void` | `undefined` | Callback invoked upon successful fetch. |
| `onError` | `(err: any) => void` | `undefined` | Callback invoked upon fetch failure. |
